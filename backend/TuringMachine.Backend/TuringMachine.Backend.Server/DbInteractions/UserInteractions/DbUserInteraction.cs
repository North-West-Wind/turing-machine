...
namespace TuringMachine.Backend.Server.DbInteractions.UserInteractions
{
    internal static class DbUserInteraction
    {
        ...
        {
            using IEnumerator<DbUser> users = db.Users.Where(user => user.Username == username).GetEnumerator();
            if (users.MoveNext()) return ServerResponse.StartTracing<string>(nameof(RegisterAndSaveUserAsync) , USER_EXISTED);
            ...
            if (validateLicenceKeyResponse.Status is not SUCCESS)
                return validateLicenceKeyResponse.WithThisTraceInfo<string>(nameof(RegisterAndSaveUserAsync) , BACKEND_ERROR);
            ...
            if (generateAccessTokenResponse.Status is not SUCCESS)
                return generateAccessTokenResponse.WithThisTraceInfo<string>(nameof(RegisterAndSaveUserAsync) , BACKEND_ERROR);
