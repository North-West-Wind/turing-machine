...
import { download, saveToCloud, upload } from "../../../helpers/network";
...
export default function DesignerLevelActions() {
	...
	const selectLevel = () => {
		...
		setLoading(true);
		saveToCloud(simulator.save()).catch(console.error);
		navigate("/level");
	};
	...
	const uploadMachine = async () => {
		setLoading(true);
		try {
			const id = await upload(simulator.save());
	...
	const importMachine = async () => {
		...
		try {
			...
			const machine = await download(id);
